...
1. リポジトリの **Settings → Secrets and variables → Actions** で
   `LINE_CHANNEL_ACCESS_TOKEN` を登録する（一度だけ）
2. `line-richmenu/.deploy-request` に `inspect` / `deploy` / `clear` のどれかを書いて push する
3. Actions の「リッチメニューをLINEに反映」が動く

トークンはSecretsに入るので、手元に置かなくてよい。
まず `inspect` で疎通を見てから `deploy` にするのが安全。
...
タブの切替（`richmenuswitch`）は、**ユーザー単位でリンクされたリッチメニュー**で動く仕様。
デフォルトリッチメニューのままでは切替が効かない、と理解している
（この点はテストアカウントで実際に押して確かめること）。自分のIDに貼って確認する:
